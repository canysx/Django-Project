    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request,"login.html",{'msg':"用户已存在",'register_form':register_form})
            ...
            user_message.message = "欢迎注册Canysx"
            user_message.save()
...
class ModifyPwdView(View):
    ...
    def post(self,request):
        ...
        if modify_form.is_valid():
            pwd1 = request.POST.get("pawword1","")
            pwd2 = request.POST.get("pawword2","")
            email = request.POST.get("email","")
            if pwd1 != pwd2:
